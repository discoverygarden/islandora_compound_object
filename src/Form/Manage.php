...
class Manage extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, AbstractObject $object = NULL) {
    ...
    if (($config->get('islandora_compound_object_compound_children') && in_array(ISLANDORA_COMPOUND_OBJECT_CMODEL, $object->models)) || !$config->get('islandora_compound_object_compound_children')) {
      ...
      if (!empty($children)) {
        ...
        $header = ['title' => $this->t('Title'), 'pid' => $this->t('Object ID')];
        $form['children']['remove_children'] = [
          '#type' => 'tableselect',
          '#title' => $this->t('Children'),
          '#header' => $header,
          '#options' => $children,
        ];
        $form['reorder_fieldset'] = [
          '#type' => 'details',
          '#title' => $this->t('Reorder'),
          '#open' => FALSE,
        ];
        $form['reorder_fieldset']['table'] = islandora_compound_object_get_tabledrag_element($object);
      }
    }
    // Add parents.
    $form['add_to_parent'] = [
    ...
    if (!empty($parent_part_of)) {
      ...
      foreach ($parent_part_of as $parent) {
        ...
        $parents[$pid] = ['title' => $parent_object->label, 'pid' => $pid];
      }

      $form['parents']['unlink_parents'] = [
        '#type' => 'tableselect',
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    drupal_set_message($this->t('Compound relationships modified.'));
  }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($form_state->getValue('child'))) {
      if ($config->get('islandora_compound_object_compound_children')) {
        ...
        if (!$compound) {
          $form_state->setErrorByName('child', $this->t('This object is not a compound object'));
        }
      }
      // Do not allow child of self.
      if ($form_state->getValue('child') == $object->id) {
        $form_state->setErrorByName('child', $this->t('An object may not be a child of itself.'));
      }
      ...
      if ($child_object) {
        ...
        foreach ($child_part_of as $part) {
          if ($part['object']['value'] == $object->id) {
            $form_state->setErrorByName('child', $this->t('The object is already a parent of the child.'));
          }
        }
      }
      else {
        $form_state->setErrorByName('child', $this->t('Invalid object supplied.'));
      }
    }
    // Parent.
    if (!empty($form_state->getValue('parent'))) {
      ...
      if (!$parent_object) {
        $form_state->setErrorByName('parent', $this->t('Invalid object supplied.'));
      }
      else {
        if ($config->get('islandora_compound_object_compound_children')) {
          if (!in_array(ISLANDORA_COMPOUND_OBJECT_CMODEL, $parent_object->models)) {
            $form_state->getErrorByName('parent', $this->t('The parent object (@pid) is not a compound object!', ['@pid' => $form_state['values']['parent']]));
          }
        }
        // Do not allow parent of self.
        if ($form_state->getValue('parent') == $object->id) {
          $form_state->setErrorByName('parent', $this->t('An object may not be the parent of itself.'));
        }
        ...
        foreach ($parent_part_of as $part) {
          if ($part['object']['value'] == $form_state->getValue('parent')) {
            $form_state->setErrorByName('parent', $this->t('The object is already a child of the parent.'));
          }
        }
      }
    }
    if (!empty($form_state->getValue('parent')) && !empty($form_state->getValue('child')) && $form_state->getValue('parent') == $form_state->getValue('child')) {
      $form_state->setErrorByName('child', $this->t('An object may not be the parent and child of the same object.'));
